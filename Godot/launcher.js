...
async function boot(){document.body.dataset.boot='loading';fit();const script=document.createElement('script');script.src='../_engine/godot.js';await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error('Player download timed out')),30000);script.onload=()=>{clearTimeout(timer);resolve();};script.onerror=()=>{clearTimeout(timer);reject(new Error('Player download failed'));};document.head.appendChild(script);});
 ...
 const engine=new Engine({...config,canvas});$('boot-message').textContent='Downloading game files…';let slow=setTimeout(()=>{if(!ready&&!failed)$('boot-detail').textContent='Large games can take longer on the first launch. Keep this tab open while the download finishes.';},30000);
 try{await engine.startGame({onProgress(current,total){if(failed)return;if(total>0){$('boot-progress').value=current/total*100;$('boot-message').textContent=current>=total?'Preparing the game…':'Downloading game files…';$('boot-detail').textContent=(current/1048576).toFixed(1)+' / '+(total/1048576).toFixed(1)+' MB';}}});ready=true;document.body.dataset.boot='ready';$('boot').hidden=true;makeControls();fit();canvas.focus();}finally{clearTimeout(slow);}
}
